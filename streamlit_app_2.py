...
if ref_file and stu_file:
    ...
    # ============================
    st.subheader("🎹 Chroma Feature Comparison")
    fig4, ax4 = plt.subplots(2, 1, figsize=(10, 6))
    librosa.display.specshow(ref_feat['chroma'], y_axis='chroma', x_axis='time', sr=sr_ref, ax=ax4[0])
    ax4[0].set(title="Reference Chroma", ylabel="Pitch Class")
    librosa.display.specshow(stu_feat['chroma'], y_axis='chroma', x_axis='time', sr=sr_stu, ax=ax4[1])
    ax4[1].set(title="Student Chroma", xlabel="Time (s)", ylabel="Pitch Class")
    st.pyplot(fig4)
